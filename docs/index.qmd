...
The recommended method for installing *spaceprime* is through `conda` (or `mamba`, a faster drop-in replacement for `conda`). Due to the package's reliance on *msprime*, the `pip` installation is only available on unix-based systems (MacOS, Linux).  

[gdal](https://gdal.org/) is a required dependency for *spaceprime*, and needs to be installed before installing *spaceprime*:
