...
#### `plot_model()`
`plot_model()` plots the deme sizes on a [folium](https://python-visualization.github.io/folium/latest/index.html) interactive  map, with precise deme sizes and outgoing migration rates for each deme present in a popup. *Note*- the map below is rendered as an image, but the actual plot is interactive.
...
We love contributions! Please see the [Contributing Guide](contributing.md) for more information.  
